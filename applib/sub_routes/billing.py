...
def index() -> str:
    ...
    with sql_cursor() as db:
        ...
        qry = (
            ...
            .outerjoin(Invoice, Invoice.id == RecurrentBill.invoice_id)
            .outerjoin(
                recent_payments,
                recent_payments.c.invoice_id == RecurrentBill.invoice_id,
            )
            .join(Client, Client.id == RecurrentBill.client_id)
        ...
        agg_calculation = (
            ...
            .outerjoin(Payment, Payment.invoice_id == RecurrentBill.invoice_id)
            .group_by(RecurrentBill.invoice_id)
            .where(RecurrentBill.date_created.between(start, stop))
            .all()
        )
    ...
    for row in agg_calculation:
        if not row.amount:
            if row.payment_status == 0:
                potential += float(row.amount_expected)

            elif row.payment_status == -1:
                cancelled += float(row.amount_expected)
            ...
            paid += float(row.amount)
            potential = float(row.amount_expected) - float(row.amount)
    ...
    return render_template(
        ...
        payment_stages={"paid": paid, "cancelled": cancelled, "potential": potential},
    )
